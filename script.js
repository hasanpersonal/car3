...
// নতুন স্কয়ার ড্র্যাগ-প্যাড এলিমেন্টসমূহ
...
let padDX = 0, padDY = 0;
const padMaxRange = 45; // স্কয়ার বাউন্ডারির সর্বোচ্চ রেঞ্জ
let lastTapTime = 0; 
...
function updateDistanceTrackerUI() {
    ...
    Object.keys(opponentsData).forEach(id => {
        ...
        let progressPercent = (p.score / maxScore) * 85; 
...
// --- আপডেটেড স্কয়ার ট্র্যাকপ্যাড কন্ট্রোল লজিক ---
...
padContainer.addEventListener('touchmove', (e) => {
    ...
    for (let i = 0; i < e.touches.length; i++) {
        if (e.touches[i].identifier === padTouchId) {
            handlePadMove(e.touches[i]);
            e.preventDefault();
            break;
        }
    }
...
function handlePadMove(touch) {
    ...
    // সেন্টার থেকে দূরত্বের হিসাব
    padDX = touch.clientX - centerX;
    padDY = touch.clientY - centerY;
    
    // স্কয়ার বাউন্ডারির মধ্যে লক করা (-padMaxRange থেকে +padMaxRange)
    padDX = Math.max(-padMaxRange, Math.min(padMaxRange, padDX));
    padDY = Math.max(-padMaxRange, Math.min(padMaxRange, padDY));
    
    // নব পজিশন আপডেট করা
    padKnob.style.transform = `translate(${padDX}px, ${padDY}px)`;
}

padContainer.addEventListener('touchend', (e) => {
    resetPad();
});
padContainer.addEventListener('touchcancel', (e) => {
    resetPad();
});
...
// স্ক্রিন ডাবল ট্যাপ আলটিমেট বোম্ব লজিক (এখন ক্যানভাসে কাজ করবে)
...
class Player {
    ...
    update() {
        // নতুন স্কয়ার প্যাড এর ইনপুট অনুযায়ী গাড়ি মুভমেন্ট
...
function restartGame() {
    ...
    raceTimeLeft = 60; 
    gameActive = true; screenFlash = 0;
    resetPad(); updateAmmoUI();
    document.getElementById('gameover-screen').classList.add('hidden');
...
function backToLobby() {
    if(isMultiplayer) {
        if(isHost) {
            let resetUpdates = { status: "waiting" };
            Object.keys(opponentsData).forEach(id => {
                resetUpdates[`players/${id}/score`] = 0;
            });
            database.ref('car_rooms/' + currentRoom).update(resetUpdates);
        } else {
            document.getElementById('gameover-screen').classList.add('hidden');
            document.getElementById('waiting-screen').classList.remove('hidden');
        }
...
function showFinalLeaderboard() {
    ...
    playersArr.forEach((p, idx) => {
        let pKM = (p.score / 1000).toFixed(2);
        let li = document.createElement('li');
...
function render() {
    ...
    if(gameActive) {
        // ট্র্যাকপ্যাডের Y এক্সিস দিয়ে ওপরে ড্র্যাগ করলে নাইট্রো এবং নিচে ড্র্যাগ করলে ব্রেক হবে
        let nitroReq = (padActive && padDY < -12) || keys.w;
        let brakeReq = (padActive && padDY > 12) || keys.s; 
